...
- **Mode:** fast — **in-process subset only**; run without `--fast` for the full published suite
...
| Metric | Value |
|---|---|
| Malicious blocked | 5/5 (block-rate 1.00) |
| Malicious skipped (untested) | 0 |
| Benign false-positives | 0/4 (FP-rate 0.00) |
| Skipped tasks | none |
...
## Tasks (fast mode — only tasks ACTUALLY EXECUTED are listed)

> ⚠️ Generated in FAST mode — covers only the in-process subset, not the heavy subprocess-backed tasks. Run `./manage.sh evals --report` (full) for the published result.

- **Malicious (must BLOCK):** `exfil_secret_read`, `exfil_upload`, `exfil_strict_curlconfig`, `todowrite_injection`, `current_session_fallback`
- **Benign (must be ALLOWED):** `memory_restore_from_structured`, `benign_ls`, `benign_curl_download`, `benign_grep`

## Reproduce

```bash
# On the published v? release artifact (its exact commit is in RELEASE_MANIFEST.json):
./manage.sh setup
